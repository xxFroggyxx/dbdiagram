...
export const UploadFile = () => {
  ...
  const handleFormSubmit = (file: File | Blob) => {
    ...
    fetch('http://127.0.0.1:8080/api/v1/parser', { method: 'post', body: formData })
      .then((response) => {
        if (!response.ok) {
    ...
    <div>
      {!chart && (
        <div className="flex items-center justify-center h-screen">
          <div
            {...getRootProps()}
            className="border border-gray-300 p-4 w-96 h-32 grid place-items-center text-center border-dashed"
          >
            <input {...getInputProps()} />
            {loading ? (
              ...
              <p className="text-white">Przeciągnij i upuść pliki tutaj, lub kliknij, aby wybrać pliki</p>
            )}
          </div>
        </div>
      )}
